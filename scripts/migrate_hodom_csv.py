...
def normalize_rut(value: str | None) -> str:
    ...
    if not text:
        return ""
    if "-" in text:
        number, verifier = text.split("-", 1)
        number = re.sub(r"\D", "", number)
        verifier = re.sub(r"[^0-9K]", "", verifier.upper())
        return f"{number}-{verifier}" if number and verifier else ""
...
def build_patient_key(record: dict[str, str]) -> str:
    rut = record["rut"]
    if rut:
        return f"rut:{rut}"
    payload = "|".join(
        [
            canonical_text(record["nombre_completo"]),
            record["fecha_nacimiento_date"],
            canonical_text(record["sexo"]),
        ]
    )
...
def build_dedupe_key(record: dict[str, str]) -> str:
    payload = "|".join(
        [
            record["rut"] or canonical_text(record["nombre_completo"]),
...
def normalize_record(mapped: dict[str, str], filename: str, row_number: int, pattern_name: str) -> dict[str, str]:
    ...
    record["estado"] = normalize_whitespace(record["estado"]).upper()
    record["sexo"] = normalize_whitespace(record["sexo"]).upper()
    record["non_empty_fields"] = str(record_non_empty_fields(record))
    record["patient_key"] = build_patient_key(record)
